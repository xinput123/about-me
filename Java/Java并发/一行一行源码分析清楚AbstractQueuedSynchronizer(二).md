...
> 这里的阻塞队列如果叫做 同步队列（sync queue）其实比较贴切，不过为了和前篇呼应，我就继续使用阻塞队列了。记住这里的两个概念，**阻塞队列** 和 **条件队列**。

![](https://github.com/xinput123/about-me/blob/main/Java/Java%E5%B9%B6%E5%8F%91/image/aqs2-1.png)
...
- 3、每个 condition 有一个关联的 **条件队列**，如线程1调用 condition1.await() 方法即可将当前线程1包装成 Node 后加入到条件队列中，然后阻塞在这里，不继续往下执行，条件队列是一个单项链表。

- 4、调用 condition.signal() 触发一次唤醒，此时唤醒的是队头，会将 condition1 对应的条件队列的 firstWaiter(队头) 移到 **阻塞队列的队尾**， 等待获取锁，获取锁后 await 方法才能返回，继续往下执行。

上面的 2 -> 3 -> 4 描述了一个最简单的流程，没有考虑中断、signAll、还有带超时参数参数的 await 方法等，不过把这里弄懂是这节的主要目的。
...
```
// 首先，这个方法是可以被中断的，不可被中断的是另一个方法 awaitUninterruptibly()
// 这个方法会阻塞，直到调用 signal 方法 (指 signal() 和 signAll()，下同)，或被中断
...
private Node addConditionWaiter() {
    ...
    // 为什么这里把 waitStatus 不等于 Node.CONDITION , 就判定为该节点发生了取消
...
在 addWaiter 方法中，有一个 unlinkCancelledWaiters() 方法，该方法用于清除队列中已经取消等待的节点。

当 await 的时候，如果发生了取消操作(这点之后会说)，或者是在节点入队的时候，发现最后一个节点是被取消的，会调用一次这个方法。
...
### 2、完全释放独占锁
回到 wait 方法，节点入队了以后，会调用 int savedState = fullyRelease(node)；方法释放锁，注意，这里是完全释放独占锁(full release)，因为 ReentrantLock 是可以重入的。
...
// 对于最简单的操作：先 lok.lock()，然后 condition1.await()。
...
// 前面提到， signl 的时候需要将节点从条件队列转移到阻塞队列
// 这个方法就是判断 node  是否已经移动到阻塞队列了
final boolean isOnSyncQueue(Node node) {

    // 移动过去的时候，node 的 waitStatus 会置为 0 ，这个之后在说 signal 方法的时候会说到
    // 如果 waitStatus 还是 Node.CONDITION, 也就是 -2，那肯定就是孩子啊条件队列中
    // 如果 node 的前驱节点 prev 还是指向 null，说明肯定没有在 阻塞队列中(prev 是阻塞队列链表中使用的)
    if (node.waitStatus == Node.CONDITION || node.prev == null)
        return false;
        
    // 如果 node 已经有后继节点 next 的时候，那就说明肯定在阻塞队列了    
    ...
    // 这个可以看上篇AQS的入队方法，首先设置的是 node.prev 指向 tail
    // 然后试试 CAS 操作将自己设置为新的 tail, 可是这次的 CAS 是可能失败的。
...
```
// 唤醒等待了最久的线程
// 其实就是，将这个线程对应的Node 从条件队列转移到阻塞队列
...
private void doSignal(Node first) {
    do {
        // 将 firstWaiter 指向 first 节点后面的第一个，因为 first 节点马上就要离开了
        // 如果将 first 移除后，后面没有节点在等待了，那么需要将 lastWaiter 设置为 null、
        if ( (firstWaiter = first.nextWaiter) == null)
            lastWaiter = null;
        // 因为 first 马上就要被转移到阻塞队列，和条件队列的链接关系在这里断掉    
        first.nextWaiter = null;
    } while (!transferForSignal(first) &&
             (first = firstWaiter) != null);
         // 这里 while 循环，如果 first 转移不成功，那么选择 first 后面的第一个节点进行转移，依次类推    
}

// 将节点从条件队列转移到阻塞队列
//  true 表示转移成功
// false 代表在 signal 之前，节点已经被取消了
final boolean transferForSignal(Node node) {
    // CAS 如果失败，说明此 node 的 waitStatus 已经不是 Node.CONDITION ，说明节点已经被取消，
    // 既然已经取消，也就不需要转移了，方法返回，转移到后面一个节点
    // 否则，将 waitStatus 设置为 0
...
// 返回true：如果此线程在 signal 之前被取消
final boolean transferAfterCancelledWait(Node node) {
    // 用CAS将阶段状态设置为 0
    // 如果这步 CAS 成功，说明是 signal 方法之前发生的中断，因为如果 signal 先发生的话，signal 会将 waitStatus 设置为 0
    if (compareAndSetWaitStatus(node, Node.CONDITION, 0)) {
        // 将节点放入阻塞队列
